        ...
        if (reportsResponse.ok) {
          ...
          if (reports.length > 0) {
            ...
            if (username !== firstReport.employee_id) {
              ...
              console.log(`   - 原因: 用户 ${username} 的日报数据与当前用户不匹配`);
              console.log('\n🔧 建议解决方案:');
              console.log('A. 用户登录的工号与日报数据不匹配');
              console.log('B. 需要检查数据库中的用户数据');
              console.log('C. 可能需要用正确的用户名重新登录');
            } else {
              console.log('✅ 权限验证正常，数据匹配');
            }
          }
        } else {
